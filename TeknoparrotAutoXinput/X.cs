...
namespace XInput.Wrapper
{
	public static class X
	{
		...
		public static Dictionary<string, int> flagMapping = new Dictionary<string, int>
					{
						{ "Dpad_Up", 0x0001 },
						{ "Dpad_Down", 0x0002 },
						{ "Dpad_Left", 0x0004 },
		...
		public class Gamepad
		{
			...
			public PacketState state = new PacketState();
			PacketState secret = new PacketState();
			...
			public bool Guide_down { get { return state.Gamepad.IsButtonDown(GamepadButtons.Guide); } }
			public bool Guide_up { get { return state.Gamepad.IsButtonUp(buttons, GamepadButtons.Guide); } }
			...
			public bool Update()
			{
				...
				int resultsecret = InputWrapper.XInputGetStateSecret(userIndex, ref secret);
				if (resultsecret == 0 && result == 0 && (secret.Gamepad.wButtons - state.Gamepad.wButtons) == 1024)
				{
			...
			public struct Pad
			{
				...
				public bool IsButtonDown(string ComboKey)
				{
					string[] parts = ComboKey.Split('+');
					for (int i = 0; i < parts.Count(); i++)
					{
						int keyCode = NameToFlag(parts[i]);
						if (keyCode < 0) return false;

						bool is_press = (wButtons & keyCode) == keyCode;
						if (!is_press) return false;
					}
					return true;
				}

				public int NameToFlag(string key)
				{
					// Vérifie si la clé existe dans le dictionnaire et renvoie la valeur correspondante
					if (flagMapping.ContainsKey(key))
					{
